...
import { stateUtils } from '../../core/state';
...
describe('stateUtils', () => {
  test('Deve obter uma propriedade aninhada corretamente', () => {
    const obj = { a: { b: { c: 42 } } };
    const result = stateUtils.getNestedProperty(obj, 'a.b.c');
    expect(result).toBe(42);
  });

  test('Deve retornar undefined para uma propriedade inexistente', () => {
    const obj = { a: { b: { c: 42 } } };
    const result = stateUtils.getNestedProperty(obj, 'a.b.d');
    expect(result).toBeUndefined();
  });

  test('Deve definir uma propriedade aninhada corretamente', () => {
    const obj = {};
    stateUtils.setNestedProperty(obj, 'a.b.c', 42);
