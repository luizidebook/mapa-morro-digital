...
function initializeMap() {
    map = L.map('map').setView([-13.410, -38.913], 13);
...
async function plotRouteOnMap(startLat, startLon, destLat, destLon, profile) {
    try {
        const response = await fetch(`https://api.openrouteservice.org/v2/directions/${profile}?api_key=${OPENROUTESERVICE_API_KEY}&start=${startLon},${startLat}&end=${destLon},${destLat}`);
        const data = await response.json();
        const coordinates = data.features[0].geometry.coordinates.map(([lon, lat]) => [lat, lon]);

        if (currentRoute) map.removeLayer(currentRoute); // Remove rota anterior, se existir

        currentRoute = L.polyline(coordinates, { color: 'blue' }).addTo(map);
        map.fitBounds(currentRoute.getBounds()); // Ajusta o zoom para exibir toda a rota
    } catch (error) {
